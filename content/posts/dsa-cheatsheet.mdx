...
- If maximum `n = 8` $\rightarrow 8! = 40320$ $\rightarrow 40320 \times \computeTime$ $= 1.008×10^{−5}$ seconds. So, computer will be able to compute it in reasonable time.
- If maximum `n = 100` → $100! = $  number with 158 digits $\rightarrow 100! \times \computeTime$ $\approx$ $3 \times 10^{150}$ **years**. This is $\approx 2.14 \times 10^{140}$ times longer than our universe 🪐 exists!
...
Convert **multiple nested loops into single loop**, reducing the time complexity from $O(n^2)$ or $O(n^3)$ to $O(n)$.
...
<ProblemComplexity
  tc="\(O(n\times\log 26) \rightarrow O(n)\)"
...
<ProblemComplexity tc="\(O(n\times\log n)\)" sc="\(O(n)\)" />
...
- Sort subset of elements resulting to TC optimization from $O(n\times\log n)$ to $O(n\times\log k)$ where k is heap size
...
- **Initialization** - placing elements to heaps by $O(k\times\log k)$
...
Without this pattern, sorting the whole collection is $O(n \times \log n)$, plus $O(k)$ to pick **top k** elements. The **top k** pattern achieves $O(n \times \log k)$ by selectively tracking compared elements, using a **heap**.
...
- $O(n\times\log k)$ - processing all elements
- $O(k\times\log k)$ - retrieving all `k` elements, due to reorganization after each removal.
