...
**Example**
$\gdef\computeTime{0.25 \cdot 10^{-9}\frac{seconds}{operation}}$
...
<AlgorithmSummary tc={{ latex: "$$O(n123)$$" }} sc={{ latex: "$$O(1)$$" }} />
...
- **Variable window size**
  ...
  1. **Expand the window**: Increment the `end` pointer to expand the window if conditions are met.
  1. **Process the window**: Perform required operations when the window meets criteria.
  1. **Adjust the window size**: Move the `start` pointer to adjust size and meet desired criteria.
...
Performing operations on set of intervals. Typical interval problem deals with **array of overlapping intervals** e.g. `[[1,
3], [3, 5]]`.
...
<ProblemComplexity
  tc="\(O(n\cdot\log 26) \rightarrow O(n)\)"
...
<ProblemComplexity tc="\(O(n\cdot\log n)\)" sc="\(O(n)\)" />
...
- **NOT** `~` - Inverts bits: `1 → 0`, `0 → 1`
  - `~n == -n - 1`.
    - Example: `~1 == -2`
  - `arr[~k]` - access <code>k<sub>th</sub></code> element in array from the end:
