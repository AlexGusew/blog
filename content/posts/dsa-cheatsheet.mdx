...
- If maximum `n = 8` → $8! = 40320$ $\rightarrow 40320 \times \computeTime$ $= 1.008×10^{−5}$ seconds. So, computer will be able to compute it in reasonable time.
...
<AlgorithmSummary
  tc={{ latex: "$$\\text{Doesn't affect}$$" }}
  sc={{ latex: "$$\\text{Improves to linear } - O(n)$$" }}
/>

> Backtracking is a class of algorithms for finding solutions to some computational problems, notably **constraint satisfaction problems**, that:
>
...
<div align="right">[Wikipedia](https://en.wikipedia.org/wiki/Backtracking)</div>

#### Optimization

- **Space Complexity** compared to Bute-force approach improves to $ O(n) $
- **Time Complexity** usually isn't affected
...
- Abandon next solutions (backtrack)
